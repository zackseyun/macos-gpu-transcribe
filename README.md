...
Everything runs on the GPU. The current default is Granite Speech 4.1 NAR so it is easy to compare against Cohere; switch back from the menu bar if Granite is not ready on your machine yet.
...
Granite Speech 4.1 NAR is the default for now so it can be tested quickly. The original Hugging Face PyTorch path requires CUDA + `flash_attention_2`, so this Mac app runs Granite through CrispASR's GGUF runtime instead. Cohere remains one click away in the menu and still handles homophones, technical terms, and sentence boundaries reliably at this size.
...
| "Loading model…" takes a while on first run | Downloading weights — Granite GGUF can be several GB; Cohere is ~2 GB. One-time |
