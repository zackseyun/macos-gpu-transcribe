...
Everything runs on the GPU. The current default is Granite Speech 4.1 NAR so it is easy to compare against Cohere; switch back from the menu bar if Granite is not ready on your machine yet. Granite is lazy: the app does not download or load the model on startup.
